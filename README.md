...
Для автоматизированного восстановления буквы «ё» в текстах (форматы: .txt, .md, .fb2, .epub).
...
2. Чистый файл с суффиксом `_yo` в формате, соответствующем формату входного файла (TXT, Markdown, FB2 или EPUB).
...
- Установленный модуль <code>colorama</code> (чтобы в терминале (CLI) подсвечивалось слово для которого предлагаются варианты для замены).
    ```терминал windows
